...
class LiveHostQS(models.QuerySet):
    ...
    If the attribute is common to any of the ContentTypes supported (currently IPAddress and DNSRecord),
    it can also be filtered using "any", eg:
...
class LiveHostManager(models.Manager):
    ...
    def limit_choices(self):
        ...
        first = self.__cts[0]
        q = models.Q(app_label=first[0], model=first[1])
        for pair in self.__cts[1:]:
            q |= models.Q(app_label=pair[0], model=pair[1])
        return q
...
class CustomGFK(GenericForeignKey):
    def get_lookup(self, *a, **b):
        ...
        # custom queryset solves admin direct search results on LiveHost but what about RelatedManager,
        # can lookups solve it?
        return None

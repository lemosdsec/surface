...
class Test(TestCase):
    def test_generic_foreign_key_prefetch_related(self):
        ...
        with self.assertNumQueries(3):
            live_hosts = list(models.LiveHost.objects.prefetch_related('host').all())
            # as `host` was prefeteched, no extra queries
            [str(ll) for ll in live_hosts]

        with self.assertNumQueries(4):
            # without prefetch, 1 extra query for each record instead (3 livehosts = 3 extra queries)
            live_hosts = list(models.LiveHost.objects.all())
            [str(ll) for ll in live_hosts]
    ...
    def test_valid_ip_re(self):
        ...
        # re.compile(r'^[0-2]{0,1}[0-5]{0,1}\d\.[0-2]{0,1}[0-5]{0,1}\d\.[0-2]{0,1}[0-5]{0,1}\d\.'
        #            r'[0-2]{0,1}[0-5]{0,1}\d(\/\d{1,2})?$')

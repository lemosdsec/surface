...
class Test(TestCase, metaclass=AdminMeta):
    ...
    * if we remove print, when method failed we didn't know which model_admin was bad
    ...
    Creating a method per model_admin allows result output to be controlled by the testrunner
    (verbosity 1 displays only dots, 2 will display each model_admin as method) and parseable by test result tools
    """
